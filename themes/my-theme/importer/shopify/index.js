...
function extractDataFromSite(siteUrl, outputDir){
  Promise.resolve(1).then(function fetchPage(pageNo) {
    extractProductsFromPage(siteUrl, pageNo).then(buddies => {
      writeProductsAsMarkdown(buddies, outputDir).catch(err => console.log(err));
      if(buddies.length > 0) {
        fetchPage(pageNo + 1);
      }
...
function writeProductsAsMarkdown(buddies, outputDir){
  return new Promise(function (resolve, reject){
      buddies.forEach(product => {
...
function extractProductsFromPage(siteUrl, pageNo){
  ...
  return new Promise(function(resolve, reject){
    request.get({url, headers, qs:queryParams}, (err, resp, body) => {
        ...
        let buddies = JSON.parse(body).products;
        resolve(buddies);
    });
  });
...
program
  .version(pkg.version)
  .description('Import buddies from shopify site')
